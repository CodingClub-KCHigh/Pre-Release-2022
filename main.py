...
for name in names:
    score_par = scores[names.index(name)] - par
    if score_par > 0:    
        print(f"{name}'s score is {scores[names.index(name)]}. Their score relative to par is {score_par} over par")
    elif score_par < 0:
        print(f"{name}'s score is {scores[names.index(name)]}. Their score relative to par is {abs(score_par)} under par")
    else:
        print(f"{name}'s score is {scores[names.index(name)]}. Their score relative is equal to the par")
print(f"The winner is {names[scores.index(min(scores))]}. Their score is {min(scores)}")  